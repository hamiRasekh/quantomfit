...
export function OrderItemProcessActivities({ itemIndex }: Props) {
  const fieldName = `items.${itemIndex}.processActivities` as const;
  ...
  const processActivities: ProcessActivitySelection[] = watch(fieldName) || [];
  ...
  }, [selectedCategoryForAdd, watch, setValue, fieldName]);

  const getActivitiesForRow = useCallback(
    (idx: number): Activity[] => {
      const pa = processActivities[idx];
  ...
  const toggleActivity = useCallback(
    (index: number, activityId: string) => {
      ...
      setValue(`${fieldName}.${index}.activityIds`, newIds);
    },
    [processActivities, setValue, fieldName]
  );
    ...
    <Box sx={{ mt: 2, pl: 2, borderLeft: 2, borderColor: 'divider' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2, mb: 2 }}>
        <Typography variant="subtitle2" sx={{ color: 'text.secondary' }}>
          فرایندها و فعالیت‌ها (این محصول)
        </Typography>
        ...
        {fields.map((field, index) => {
          ...
          return (
            <Box key={field.id}>
              ...
              >
                <Typography variant="body2" sx={{ minWidth: 120, pt: 1 }}>
