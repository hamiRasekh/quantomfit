...
if (typeof acceptLanguage !== 'undefined' && (acceptLanguage as any).languages) {
  (acceptLanguage as any).languages([...supportedLngs]);
}
...
export async function detectLanguage() {
  ...
  const fromCookie = cookieLang && (acceptLanguage as any).get && (acceptLanguage as any).get(cookieLang);

  // 2. Try Accept-Language header
  const headerLang = headerStore.get('accept-language') ?? undefined;
  const fromHeader =
    headerLang && storageConfig.cookie.autoDetection && (acceptLanguage as any).get && (acceptLanguage as any).get(headerLang);
