...
import { Chart, useChart, ChartSelect } from '@/components/ui/chart';
...
export function CourseHoursSpent({ title, subheader, chart, sx, ...other }: Props) {
  ...
  const chartColors = chart.colors ?? [theme.vars.palette.text.primary];
  ...
  const chartOptions = useChart({
    ...
    tooltip: {
      y: { formatter: (value: number) => `${value} h`, title: { formatter: () => '' } },
    },
    ...chart.options,
  });
