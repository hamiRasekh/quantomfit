...
import { useDateRangePicker, CustomDateRangePicker } from '@/components/ui/custom-date-range-picker';
...
export function KanbanDetails({ task, open, onUpdateTask, onDeleteTask, onClose }: Props) {
  ...
  // Safely handle due date - ensure task.due exists and has valid values
  const startDate = task.due && task.due[0] ? dayjs(task.due[0] as string) : null;
  const endDate = task.due && task.due[1] ? dayjs(task.due[1] as string) : null;
  const rangePicker = useDateRangePicker(startDate, endDate);
