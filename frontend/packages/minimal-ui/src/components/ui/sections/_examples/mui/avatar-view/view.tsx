...
import { colorKeys } from '@/ui/theme/core';
    ...
    component: (
      <ComponentBox sx={{ flexDirection: 'column' }}>
        {SIZES.slice(0, 2).map((size) => {
          const AvatarGroupAny = AvatarGroup as any;
          return (
            <Tooltip key={size as any} title={size}>
              <AvatarGroupAny
                key={size as any}
                sx={{
        ...
        <Tooltip title="compact">
          {(() => {
            const AvatarGroupAny = AvatarGroup as any;
            return (
              <AvatarGroupAny variant="compact" sx={{ width: 48, height: 48 }}>
