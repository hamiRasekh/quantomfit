...
import { getFileMeta } from '@/components/ui/file-thumbnail';
...
function parseValue(values: FieldValues, key: string) {
  ...
  if (key === 'singleUpload') {
    return JSON.stringify(values.singleUpload && getFileMeta(values.singleUpload), null, 2);
  }

  if (key === 'multiUpload') {
    return JSON.stringify(
      values.multiUpload.map((file: File) => getFileMeta(file)),
      null,
      2
    );
  }
