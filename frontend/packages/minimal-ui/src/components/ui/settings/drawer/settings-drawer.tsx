...
import { primaryColorPresets } from '@/ui/theme/with-settings';
...
export function SettingsDrawer({ sx, defaultSettings }: SettingsDrawerProps) {
  ...
  const visibility = {
    mode: false, // Disable mode toggle - always light mode
  ...
  // Force light mode - prevent any mode changes
  useEffect(() => {
    if (settings.state.mode !== 'light') {
      settings.setState({ mode: 'light' });
    }
  ...
  const handleReset = useCallback(() => {
    settings.onReset();
    setMode('light'); // Always reset to light mode
  }, [setMode, settings]);
